...
Evolutionary rates under the OU model represent a ratio of the evolutionary variance $\sigma^2$ by the attraction parameter $\alpha$ {EQUATION XX IN REF XX OR METHODS IN THIS PAPER}. Therefore, variable rates can emerge as a result from differences in either of these parameters <!-- RM: don't use process definitions directly associated with model parameters because they can represent multiple things. Removed: depend on genetic variation and strength of selection or constraint {REF}.) --> Comparing these parameters individually showed that the observed increase in divergence rate for spacing and brightness compared to diameter and hue is due to differences in $\sigma^2$ ($\sigma^2_{b-h}$ = xx [2.5, 76.7], $\sigma^2_{t-d}$ = xx [1.03, 108.9]) rather than $\alpha$ ($\alpha_{b-h}$ = xx [-156.9, 101.0], $\alpha_{t-d}$ = xx [-49.7, 460.0]; Fig. S6). <!-- need to work on making this more clear. Raf, do you think it is important to have in here? RM: maybe the values are not necessary, just reference the SI figure. -->
...
<!-- RM not sure this last paragraph I added fits here or if it should go towards the end --> 
...
### Structural traits evolve faster than morphological traits
 ...
 <!-- RM: ssq does NOT directly map to additive genetic variation. understanding the micro-macro link between parameters of evolutionary and quantgen models is the million dollar question in the field right now… This section can undermine our whole argument for most reviewers. We should not discuss in terms of process nor in polulation-level attributes… we need to figure out a way of making this argument WITHOUT quantgen & plasticity references, maybe just broad developmental handwaving? -->  
...
<!-- RM I think this  part has to go way earlier in the discussion, to validate some of our focus. It should also be much MUCH shorter, because we didn't test this --> 
